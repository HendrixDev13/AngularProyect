...
@Component({
  ...
  standalone: false,
  templateUrl: './grafico-ganancias.component.html',
  styleUrl: './grafico-ganancias.component.css'
})
export class GraficoGananciasComponent {
  ...
  ngAfterViewInit(): void {
    ...
    this.reporteSrv.getGraficoGanancias().subscribe({
      next: data => this.crearGrafico(data),
      error: () => alert('Error al cargar gráfico de ganancias')
    });
  }
  ...
  crearGrafico(data: any[]): void {
    ...
    this.chart = new Chart('graficoGanancias', {
      type: 'bar',
      data: {
        labels: labels,
        datasets: [{
